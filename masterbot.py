...
client = commands.Bot(description="Here is some command for you", command_prefix=commands.when_mentioned_or("m!"), pm_help = False)
...
async def on_ready():
	...
	return await client.change_presence(game=discord.Game(name='Master play V2 || m!help ')) #add_your_bot_status_here
...
async def info(ctx):
    ...
    await client.say('***THIS BOT IS MADE BY MASTER JOKER AND IF YOU NEED TO INVITE THIS BOT SO WRITE `m!botinvite` THANKS***')
